// src/components/sections/service-page-content.tsx
"use client";
...
export function ServicePageContent({
  ...
  service: ServiceData;
  relatedServices: { slug: string; name: string; shortName: string }[];
}) {
  ...
  return (
    <main className="min-h-screen bg-[#f5f5f3]">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 pt-28 md:pt-36">
        <nav aria-label="Breadcrumb" className="text-sm text-zinc-500 mb-8">
          <ol className="flex items-center gap-2 flex-wrap">
            ...
            <li className="text-zinc-900 font-medium">{service.name}</li>
          </ol>
        </nav>
      </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {service.benefits.map((benefit, i) => (
            <div key={i} className="flex items-start gap-3">
              <CheckCircle2 className="w-5 h-5 text-zinc-900 mt-0.5 flex-shrink-0" />
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-8 py-16 border-t border-zinc-200/60">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {service.targetCustomers.map((customer, i) => (
            <div key={i} className="flex items-start gap-3">
              <span className="w-5 h-5 rounded-full bg-zinc-100 border border-zinc-300 flex items-center justify-center flex-shrink-0 mt-0.5">
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-8 py-16 border-t border-zinc-200/60">
          ...
          <div className="flex flex-wrap gap-3">
            {service.technologies.map((tech, i) => (
              <span
                key={i}
                className="px-4 py-2 bg-white border border-zinc-200 text-zinc-700 rounded-full text-sm font-medium"
              >
                {tech}
              </span>
            ))}
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {relatedServices.map((related) => (
              <Link
                ...
                className="group bg-white border border-zinc-200/80 rounded-xl p-6 hover:border-zinc-400 transition-all"
              >
                <h3 className="text-zinc-900 font-medium mb-1 group-hover:text-zinc-600 transition-colors">
                  {related.name}
                </h3>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-8 py-20 border-t border-zinc-200/60 text-center">
        ...
        <p className="text-zinc-600 max-w-lg mx-auto mb-8">
          ...
          and timeline. No commitment required.
        </p>
        <a
          href="https://wa.me/918608113558"
          target="_blank"
